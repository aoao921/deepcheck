...
def create_event():
    ...
    system_message_content = {
        "response_text": f"系统创建了安全基线检查任务: {event.event_name or '未命名任务'}"
    }
...
def delete_event(event_id):
    ...
    try:
        # 删除相关的消息、任务、动作、命令等
        from app.models import Message, Task, Action, Command, Execution, Summary

        # 删除相关的消息
        Message.query.filter_by(event_id=event_id).delete()

        # 删除相关的总结
        Summary.query.filter_by(event_id=event_id).delete()

        # 删除相关的执行记录
        Execution.query.filter_by(event_id=event_id).delete()

        # 删除相关的命令
        Command.query.filter_by(event_id=event_id).delete()

        # 删除相关的动作
        Action.query.filter_by(event_id=event_id).delete()
